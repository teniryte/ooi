...
function getDocsIndex (sections) {
  let index = [];
  sections.forEach(section => {
    section.docs.forEach(doc => {
      let key = `${doc.type} ${doc.name} ${doc.desc} ${doc.code}`;
      index.push({
        key: key,
        doc: doc,
        section: doc.section,
  ...
  sections.forEach(section => {
    let sec = {
      ...
      docs: section.docs.map(doc => ({
        ...
        section: doc.section,
        content: '',
        call: `<span style="color: #0087e0;">${doc.name}</span> (${doc.args.map(arg => `${arg}`).join(', ')}) <span style="color: #aaa; font-weight: 600;"> => </span> <code>${doc.result}</code>`,
      })),
    };
    list.push(sec);
  });
...
async function generateList (dir, filename) {
  ...
  let contentsCode = renderContents(sections),
    ...
    result = boilerplate.replace('<!-- {{CODE}} -->', code).replace('[[indexCode]]', JSON.stringify(index, null, 2));
